...
#define CHECK(status)                                                                                                  \
    ...
    {                                                                                                                  \
        ...
        if (ret != 0)                                                                                                  \
        {                                                                                                              \
            trtx::gLogError << "Cuda failure: " << ret << std::endl;                                                 \
            abort();                                                                                                   \
        }                                                                                                              \
    } while (0)
...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                                                                        \
    ...
    {                                                                                                                  \
        if (!(status))                                                                                                 \
        {                                                                                                              \
            trtx::gLogError << errMsg << " Error in " << __FILE__ << ", function " << FN_NAME << "(), line " << __LINE__     \
                      << std::endl;                                                                                    \
            return val;                                                                                                \
        }                                                                                                              \
    } while (0)
...
{
    const auto inds = trtx::argMagnitudeSort(output.cbegin(), output.cend());
...
{
    std::vector<size_t> indices = trtx::argMagnitudeSort(v.cbegin(), v.cend());
    indices.resize(k);
    return indices;
}
...
template <int C, int H, int W>
void readPPMFile(const std::string& filename, trtx::PPM<C, H, W>& ppm)
{
...
inline void loadLibrary(const std::string& path)
{
    ...
    {
#ifdef _MSC_VER
        trtx::gLogError << "Could not load plugin library: " << path << std::endl;
#else
        trtx::gLogError << "Could not load plugin library: " << path << ", due to: " << dlerror() << std::endl;
#endif
    }
}
